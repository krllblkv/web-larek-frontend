...
export class FormModel {
    ...
    private validateField(field: keyof IOrderDetails, value: string): string | null {
        switch (field) {
            case 'payment':
                return value ? null : 'Выберите способ оплаты';
                
            case 'address':
                return value && value.trim().length >= 5 ? null : 'Введите корректный адрес (минимум 5 символов)';
                
            case 'email':
                const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
                return value && emailRegex.test(value) ? null : 'Введите корректный email';
                
            case 'phone':
                const phoneRegex = /^\+?[0-9]{10,15}$/;
                return value && phoneRegex.test(value) ? null : 'Введите корректный телефон (10-15 цифр)';
